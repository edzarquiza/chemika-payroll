...
st.set_page_config(page_title="Chemika Tools", page_icon="📊", layout="wide")
...
st.title("📊 Chemika Tools")

tab1, tab2 = st.tabs(["📋 Payroll Timesheet Extractor", "🧾 Invoice TXT Formatter"])


# ══════════════════════════════════════════════════════════════════════════════
# TAB 1 — Payroll Timesheet Extractor (original app — unchanged)
# ══════════════════════════════════════════════════════════════════════════════
...
def extract_name_from_filename(filename):
    ...
    if len(parts) >= 2:
        ...
        setstring = parts[0] if parts else ""
...
def match_employee(setstring):
    ...
    for emp in EMPLOYEES:
        ...
        if surname.lower() == setstring_clean:
            return emp
        full = (surname + " " + initial).strip().lower()
        if full == setstring_clean:
            return emp
    return None
...
def parse_surname_initial(filename):
    ...
    for p in parts:
        ...
        if p_clean in month_names or p.isdigit():
            break
        name_parts.append(p)
...
def process_file(file_bytes, filename):
    ...
    # ─── CHECK 1: Filename vs sheet name mismatch ───
    ...
    # ─── CHECK 2: Parse header STD hours for cross-check ───
    ...
    for row in range(6, scan_end):
        ...
        # ─── CHECK 6: Working day with hours but no sign-in ───
    ...
    # ─── CHECK 7: Header STD vs Summary STD mismatch ───
    ...
    # ─── CHECK 8: STD MONTHLY HOURS missing ───
    if std_hrs == 0:
        errors.append("**STD MONTHLY HOURS is 0 or missing** — cannot verify hours")

    # ─── CHECK 9: Daily OT vs Summary OT mismatch ───
    summary_ot = ot10 + ot15 + ot20
    if daily_ot_total > 0 or summary_ot > 0:
        if abs(daily_ot_total - summary_ot) > 0.5:
    ...
    # ─── CHECK 10: Low hours warning ───
    if std_hrs > 0 and total_daily_hours > 0:
        if total_daily_hours < std_hrs * 0.5:
...
def build_output(results, month_label):
    ...
    header_fill = openpyxl.styles.PatternFill('solid', fgColor='D9E1F2')
    ...
    for r in results:
        if r["matched"]:
            if r.get("is_new"):
                ...
                key = r["employee"]["surname"].strip().lower()
                result_map[key] = r
    ...
    last_existing_row = max(e["row"] for e in EMPLOYEES)
    ...
    total_rows = last_existing_row + len(new_results)

    col_widths = {1: 5, 2: 18, 3: 4, 4: 12, 5: 8, 6: 12, 7: 8, 8: 10, 9: 10,
                  10: 12, 11: 10, 12: 12, 13: 8, 14: 8, 15: 8, 16: 16, 17: 12,
                  18: 16, 19: 12, 20: 10, 21: 8, 22: 8}
    ...
    warn_fill_style = openpyxl.styles.PatternFill('solid', fgColor='FFEB9C')
    ...
    for r in sorted(all_r, key=lambda x: x["employee"]["surname"]):
        ...
        for warn in r.get("warnings", []):
            ...
            ws_issues.cell(issue_row, 3, clean)
            for c in range(1, 4):
                ws_issues.cell(issue_row, c).fill = warn_fill_style
...
with tab1:
    ...
    if uploaded_files:
        ...
        if st.button("🚀 Extract & Generate Summary", type="primary", use_container_width=True):
            ...
            if new_employees:
                st.info(f"**{len(new_employees)} new employee(s)** detected — added to output (highlighted yellow, category blank):")
            ...
            if all_results:
                ...
                output = build_output(results, month_label)
...
def build_txt(df: pd.DataFrame, memo: str, due_date: int, due_days: int, tax_code: str, account: str) -> bytes:
    ...
    header = TAB.join([
        "Date", "Sub Total", "Other", "GST", "Company Name",
        "Invoice Number", "Memo", "TT Ex GST", "TT Inc GST",
        "Due Date", "Due Days", "Tax Code", "Account",
    ])
    blank = TAB * 12
    ...
    if inv_file:
        try:
            if inv_file.name.endswith(".csv"):
                ...
                df_inv = pd.read_excel(inv_file, sheet_name="Sheet1")
            ...
            st.success(f"✅ **{row_count} invoices** processed across **{companies} companies** · Sorted by Company → Invoice Number · Ready to download as `{out_filename}`")
        ...
        st.info("Upload an invoice spreadsheet above to begin. Accepts `.xlsx` or `.csv` — must include columns: Date, Sub Total, GST, Company Name, Invoice Number.")
